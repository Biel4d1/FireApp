...
func (g *Generator) buildBittersweetHook() {
	...
	baseG := 55.0 // Lower register base (G3) to prevent piercing high notes
	...
	// Smooth G minor progression without dissonant intervals
	g.phraseMotif[0] = baseG + 7.0  // D4
	g.phraseMotif[4] = baseG + 3.0  // Bb3
	g.phraseMotif[8] = baseG + 7.0  // D4
	g.phraseMotif[12] = baseG + 10.0 // F4

	g.phraseMotif[16] = baseG + 10.0 // F4
	g.phraseMotif[20] = baseG + 7.0  // D4
	g.phraseMotif[24] = baseG + 3.0  // Bb3
	g.phraseMotif[28] = baseG + 5.0  // C4

	g.phraseMotif[32] = baseG + 7.0  // D4
	g.phraseMotif[36] = baseG + 10.0 // F4
	g.phraseMotif[40] = baseG + 7.0  // D4
	g.phraseMotif[44] = baseG + 3.0  // Bb3

	g.phraseMotif[48] = baseG + 5.0  // C4
	g.phraseMotif[52] = baseG + 3.0  // Bb3
	g.phraseMotif[56] = baseG        // G3 Root
	g.phraseMotif[60] = baseG + 2.0  // A3
...
func (g *Generator) SampleStateAtTime(t float64) (float64, float64, float64, float64, float64, float64) {
	...
	if leadMIDI > 0 {
		...
		for leadMIDI > 67.0 { // Strictly cap lead at G4 (~392 Hz) to prevent high screeches
			leadMIDI -= 12.0
		}
		for leadMIDI < 43.0 {
			leadMIDI += 12.0
